...
function saveProgress() {
  const workoutDate = workoutData?.date || null;
  localStorage.setItem(PROGRESS_KEY, JSON.stringify({ date: workoutDate, sets: Array.from(completedSets) }));
}
...
function loadProgress() {
  try {
    ...
    if (saved) {
      ...
      // Support legacy format (plain array) and new format ({ date, sets })
      if (Array.isArray(parsed)) {
        // Old format — discard; can't verify which workout it belongs to
        completedSets = new Set();
        localStorage.removeItem(PROGRESS_KEY);
      } else if (parsed && parsed.date && parsed.date === (workoutData?.date || null)) {
        completedSets = new Set(parsed.sets || []);
      } else {
        // Workout has changed — clear stale progress
        completedSets = new Set();
        localStorage.removeItem(PROGRESS_KEY);
      }
    }
